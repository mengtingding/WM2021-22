...
m.addConstrs((yvars[j,k] <= demand[k]*wvars[j] for j in warehouses for k in customers), "Warehouse Capacity")
m.addConstrs((xvars[i,j] <= capacity[i]*wvars[j] for i in plants for j in warehouses), "Bounds on X's")
