...
def read_cities(filename):
    ...
    """
    filename = "cities.csv"
    reader = csv.reader(open(filename, "rt")) # may raise IOError
...
class City:
    ...
    def distance(self, city):
        xDis = math.radians(self.x - city.x)
        yDis = math.radians(self.y - city.y)
        a = math.pow(math.sin(xDis/2), 2) \
           + math.cos(math.radians(self.x)) * math.cos(math.radians(city.x)) * pow(math.sin(yDis/2), 2)
